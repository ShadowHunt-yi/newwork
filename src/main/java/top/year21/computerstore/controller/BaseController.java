...
public class BaseController {
     ...
     **/
    //@ExceptionHandler()用于统一处理抛出的异常，凡是参数列表中的异常都会被处理
    //1.被@ExceptionHandler参数修饰的方法，可以自动将异常对象传递给此方法的参数列表上，这里用Throwable e 来接收
    //2.这个方法的放回值就是需要传递给前端的数据
    //再次说明：当项目中产生了异常，被统一拦截到此方法中，这个方法就充当的时请求处理方法，方法的返回值直接传到前端
     ...
     **/
    public final String getUsernameFromSession(HttpSession session){  //final不能被修改
