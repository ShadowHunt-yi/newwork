...
public class IUserServiceImpl implements IUserService {
     ...
     **/
    @Override
    public void userRegister(User user) {// 通过User参数来获取传递过来的username
        //需要先判断用户名是否在数据库中重复
        ...
        //密码加密的实现形式：md5算法的形式：67jfsgsdfg-dasfas13-34asddfas-wedetqw-454556
        ...
        //使用UUID获取时间戳创建盐值，随机生成一个盐值。
        String salt = UUID.randomUUID().toString().toUpperCase();//将密码和盐值作为一个整体进行加密操作
    ...
    public User userLogin(User user) {
        ...
        String salt = loginUser.getSalt();//先获取数据库中的盐值
        String databasePwd = loginUser.getPassword();//获取到数据库中用户加密后的密码
        Integer deleteStatus = loginUser.getIsDelete();//获取状态信息

        //对用户输入的密码进行加密，忽略原有密码的强度，提升了安全性
        String md5PasswordBy = PasswordEncryptedUtils.getPasswordByMD5(userPassword, salt);

        //将用户登录输入得到密码进行加密后的字符和数据库查询的MD5进行校验
    ...
    public void userResetPwd(String originalPassword,String newPassword, HttpSession session) {
        ...
        //对用户输入的密码进行加密
        String md5Password = PasswordEncryptedUtils.getPasswordByMD5(originalPassword, salt);//这是用户在修改密码界面输入的原始密码
